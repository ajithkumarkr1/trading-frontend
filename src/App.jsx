...
function App() {
    ...
    let eventSource;
    try {
        eventSource = new EventSource("http://127.0.0.1:5000/api/stream-logs");
    ...
    const handleConnectBroker = async (e) => {
        ...
        try {
            const response = await fetch('http://localhost:5000/api/connect-broker', {
                method: 'POST',
...
const handleTradeToggle = async (index) => {
    ...
    if (currentStatus === 'active') {
        try {
            ...
            const response = await fetch("http://127.0.0.1:5000/api/disconnect-stock", {
                method: "POST",
                headers: {
    ...
    const handleStartAllTrades = async () => {
        ...
        try {
            ...
            const tradeResponse = await fetch('http://localhost:5000/api/start-all-trading', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
    ...
    const handleCloseAllPositions = async () => {
        try {
            const response = await fetch("http://127.0.0.1:5000/api/close-all-positions", {
                method: "POST",
                headers: { "Content-Type": "application/json" }
            });
    ...
    const handleStockSymbolChange = async (e, index) => {
        ...
        try {
            const response = await fetch(`http://localhost:5000/api/get-lot-size?symbol=${encodeURIComponent(newStockName)}`);
            const data = await response.json();
